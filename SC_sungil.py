...
def load_images(img_dir, train_annot_dir, test_annot_dir, cls_dir):
    ...
    elif img_dir == 'additional images/':
        
        sc_data = np.array([cv2.imread(filename) for filename in glob.glob('additional images/*.jpg')])
        data_labels = np.array([filename[18:-4] for filename in glob.glob('additional images/*.jpg')])
